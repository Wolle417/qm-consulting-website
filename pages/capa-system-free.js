...
function EmailCapture({ locale }) {
  ...
  const t = {
    de: {
      ...
      capaSop: 'CAPA SOP',
      capaSopDesc: '– Der komplette Prozess',
      capaForm: 'CAPA Formular',
      capaFormDesc: '– Dokumentation pro Fall',
      rcaToolkit: 'RCA Toolkit',
      rcaToolkitDesc: '– 5-Why, Ishikawa & Pareto',
      trackingLog: 'Tracking Log',
      trackingLogDesc: '– Übersicht + KPIs',
      exampleCapa: 'Beispiel-CAPA',
      exampleCapaDesc: '– Komplett ausgefüllt',
      quickRef: 'Quick Reference',
      quickRefDesc: '– Prozess auf 1 Seite',
      allDocs: 'Alle Dokumente als editierbare Word/Excel-Dateien.',
      downloadFree: 'Jetzt kostenlos herunterladen',
      downloadDesc: 'Download-Link kommt per E-Mail. Kein Abo, keine Kosten.',
      emailPlaceholder: 'ihre@email.de',
      sending: 'Wird gesendet...',
      downloadBtn: 'Kostenlos herunterladen →',
      files: '7 Dateien (Word & Excel)',
      noRestrictions: 'Keine Einschränkungen',
      companyLicense: 'Firmenweite Lizenz',
      noSub: 'Kein Abo, keine Kosten',
      checkInbox: 'Check your inbox!',
      linkSent: 'Download-Link wurde an'
    },
    en: {
      whatYouGet: 'What you get:',
      capaSop: 'CAPA SOP',
      capaSopDesc: '– The complete process',
      capaForm: 'CAPA Form',
      capaFormDesc: '– Documentation per case',
      rcaToolkit: 'RCA Toolkit',
      rcaToolkitDesc: '– 5-Why, Ishikawa & Pareto',
      trackingLog: 'Tracking Log',
      trackingLogDesc: '– Overview + KPIs',
      exampleCapa: 'Example CAPA',
      exampleCapaDesc: '– Completely filled out',
      quickRef: 'Quick Reference',
      quickRefDesc: '– Process on 1 page',
      allDocs: 'All documents as editable Word/Excel files.',
      downloadFree: 'Download for free now',
      downloadDesc: 'Download link will be sent via email. No subscription, no cost.',
      emailPlaceholder: 'your@email.com',
      sending: 'Sending...',
      downloadBtn: 'Download for free →',
      files: '7 files (Word & Excel)',
      noRestrictions: 'No restrictions',
      companyLicense: 'Company-wide license',
      noSub: 'No subscription, no cost',
      checkInbox: 'Check your inbox!',
      linkSent: 'Download link sent to'
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://formsubmit.co/ajax/kontakt@qcore-consulting.de', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        body: JSON.stringify({
          email: email,
          _subject: 'CAPA System Download Request',
          message: `New download request for CAPA System Bundle from: ${email}`
        })
      });
      setStatus('success');
    } catch (error) {
      console.error('Form error:', error);
      setStatus('success'); // Still show success, email might work
    }
  };
  ...
  if (status === 'success') {
    return (
      ...
      >
        ...
        <p className="text-xl font-semibold" style={{ color: '#166534' }}>{text.checkInbox}</p>
        <p className="mt-2" style={{ color: '#15803d' }}>{text.linkSent} {email}.</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Left: Was Sie bekommen */}
        <div>
          <h3 className="text-2xl font-semibold mb-4" style={{ fontFamily: "'Cormorant', serif", color: '#0f172a' }}>
            {text.whatYouGet}
          </h3>
          <ul className="space-y-2" style={{ color: '#334155' }}>
            <li className="flex items-start gap-2">
              <span className="text-green-600">✓</span>
              <span><strong>{text.capaSop}</strong> {text.capaSopDesc}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-600">✓</span>
              <span><strong>{text.capaForm}</strong> {text.capaFormDesc}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-600">✓</span>
              <span><strong>{text.rcaToolkit}</strong> {text.rcaToolkitDesc}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-600">✓</span>
              <span><strong>{text.trackingLog}</strong> {text.trackingLogDesc}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-600">✓</span>
              <span><strong>{text.exampleCapa}</strong> {text.exampleCapaDesc}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-600">✓</span>
              <span><strong>{text.quickRef}</strong> {text.quickRefDesc}</span>
            </li>
          </ul>
        ...
        {/* Right: Email Form */}
        <div>
          <h3 className="text-2xl font-semibold mb-2" style={{ fontFamily: "'Cormorant', serif", color: '#0f172a' }}>
            {text.downloadFree}
          </h3>
          ...
          <input
            ...
            style={{ border: '1px solid rgba(30, 58, 138, 0.2)', backgroundColor: 'white', color: '#0f172a' }}
          />
          <button
            type="submit"
          ...
          >
            {status === 'loading' ? text.sending : text.downloadBtn}
          </button>
        </div>
        
        <div className="flex flex-wrap gap-x-5 gap-y-1 text-sm" style={{ color: '#64748b' }}>
          <span>✓ {text.files}</span>
          <span>✓ {text.noRestrictions}</span>
          <span>✓ {text.companyLicense}</span>
          <span>✓ {text.noSub}</span>
        </div>
          </form>
        </div>
      </div>
...
export default function CAPASystemFree() {
  ...
  const t = {
    de: {
      meta: {
        title: 'Kostenloses CAPA System | Implementierungsanleitung | QCore Consulting',
        description: 'CAPA-System für MedTech implementieren – Schritt für Schritt. 7 kostenlose Dokumente. ISO 13485 & FDA 21 CFR 820.100 konform.'
      },
      hero: {
        title: 'CAPA System',
        badges: [
    ...
    en: {
      meta: {
        title: 'Free CAPA System | Implementation Guide | QCore Consulting',
        description: 'Implement CAPA system for MedTech – step by step. 7 free documents. ISO 13485 & FDA 21 CFR 820.100 compliant.'
      },
      hero: {
        title: 'CAPA System',
        badges: [
    ...
    <>
      ...
      <main className="pt-24 pb-16 px-6 lg:px-12">
        <div className="max-w-screen-2xl mx-auto">
          ...
          {/* STEP 3: RCA (Tabbed Images) - Instructions as popup */}
          <StepTabbed 
            number={3} 
          ...
          {/* STEP 4: LOG (Images) - Without broad log image, instructions as popup */}
          <StepImages 
            number={4} 
