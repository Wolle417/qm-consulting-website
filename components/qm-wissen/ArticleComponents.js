...
export const TipBox = ({ children }) => (
  <div className="bg-qcore-navy bg-opacity-30 border-l-4 border-yellow-500 p-4 my-6 rounded-r">
    <p className="text-gray-900 text-sm font-medium mb-1">💡 Praxis-Tipp</p>
...
export const InfoBox = ({ children }) => (
  <div className="bg-qcore-navy bg-opacity-30 border-l-4 border-blue-400 p-4 my-6 rounded-r">
    <p className="text-gray-900 text-sm font-medium mb-1">ℹ️ Info</p>
    <div className="text-gray-900 text-sm">{children}</div>
  </div>
);
...
export const WarningBox = ({ children }) => (
  <div className="bg-qcore-navy bg-opacity-30 border-l-4 border-yellow-500 p-4 my-6 rounded-r">
    <p className="text-gray-900 text-sm font-medium mb-1">⚠️ Wichtig</p>
    <div className="text-gray-900 text-sm">{children}</div>
  </div>
);
...
export const ExampleBox = ({ children }) => (
  <div className="bg-qcore-navy bg-opacity-30 border-l-4 border-gray-500 p-4 my-6 rounded-r">
    <p className="text-gray-900 text-sm font-medium mb-1">📌 Beispiel</p>
    <div className="text-gray-900 text-sm">{children}</div>
  </div>
);
      ...
      <thead>
        <tr className="bg-gray-700 bg-opacity-50">
          {headers.map((header, i) => (
            <th key={i} className="text-left p-3 text-gray-900 font-medium">
              {header}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {rows.map((row, i) => (
          <tr key={i} className="border-b border-gray-700">
            {row.map((cell, j) => (
              <td key={j} className="p-3 text-gray-900">
                {cell}
              </td>
            ))}
          </tr>
