# DEKRA QMB Prüfungstrainer – Artikel für QCore Consulting
...
Neuen Eintrag als **eigene Kategorie** oder unter "Grundlagen" hinzufügen:
```jsx
{ name: '🎯 QMB Prüfungstrainer', href: '/qm-wissen/qmb-trainer' }
```

#### 2. Hub-Seite prüfen
**Datei:** `/pages/qm-wissen/index.js`

Falls dort Artikel-Karten angezeigt werden, neue Karte hinzufügen (prominent platzieren!):
```jsx
{
  title: 'QMB Prüfungstrainer',
  description: '100 Fragen zur Vorbereitung auf die DEKRA QMB-Prüfung',
...
**Titel:** DEKRA QMB Prüfungstrainer  
**Breadcrumb:** QM-Wissen › Training › QMB Prüfungstrainer
...
// SEO Meta-Tags
<Head>
  <title>DEKRA QMB Prüfungstrainer: 100 Fragen zur ISO 9001 | QCore Consulting</title>
  <meta name="description" content="Kostenloser QMB Prüfungstrainer mit 100 Multiple-Choice-Fragen zur Vorbereitung auf die DEKRA Zertifizierung. ISO 9001:2015 Wissen testen und lernen." />
</Head>
```
...
### Modi
1. **Übungsmodus (practice):** Sofortiges Feedback nach jeder Frage
2. **Prüfungsmodus (exam):** 40 zufällige Fragen, 60 Minuten, Auswertung am Ende

### Features
- Zufällige Fragenreihenfolge
- Fortschrittsanzeige
- Richtig/Falsch-Feedback mit Erklärung
...
| Die 7 QM-Grundsätze | 10 |
| Kontext der Organisation (Kap. 4) | 8 |
| Führung (Kap. 5) | 10 |
| Planung (Kap. 6) | 8 |
| Unterstützung (Kap. 7) | 12 |
| Betrieb (Kap. 8) | 12 |
...
export default function QMBTrainer() {
  ...
  // Start Übungsmodus (alle 100 Fragen)
  const startPractice = () => {
  ...
  // Start Prüfungsmodus (40 zufällige Fragen, 60 Min)
  const startExam = () => {
  ...
  // ... Rest der UI-Logik (siehe vollständige Implementierung)
}
```
...
- Ausgewählt (vor Check): `bg-qcore-cyan/20 border-qcore-cyan`
- Neutral: `bg-slate-600/30 border-slate-600`

### Buttons
- Primär: `bg-qcore-cyan text-white hover:bg-qcore-cyan/80`
- Sekundär: `bg-slate-600 text-white hover:bg-slate-500`
...
- [ ] In `/components/QMWissenSidebar.js` neuen Eintrag hinzufügen

### Hub-Seite:
- [ ] In `/pages/qm-wissen/index.js` als Featured-Karte hinzufügen

### Testen:
- [ ] Alle 100 Fragen korrekt geladen
- [ ] Übungsmodus funktioniert (sofortiges Feedback)
- [ ] Prüfungsmodus funktioniert (40 Fragen, Timer, Auswertung am Ende)
- [ ] Responsive Design
...
```bash
git add .
git commit -m "Add QMB Prüfungstrainer with 100 questions"
git push
```
