...
def should_skip_section(line, current_section):
    ...
    Returns: (should_skip, new_section_state)
    new_section_state: section name to continue skipping, or None to stop skipping
    
    We only skip DOCUMENT CONTROL (duplicate info) and VERSION CONTROL.
    TABLE OF CONTENTS is kept in the document!
    """
    line_upper = line.upper().strip()
    line_trimmed = line.strip()
    
    # If we're NOT in a skip section, check if this line starts one
    if not current_section:
        # Only skip DOCUMENT CONTROL and VERSION CONTROL, NOT TABLE OF CONTENTS
        if line_upper in ['## DOCUMENT CONTROL', '## VERSION CONTROL']:
            print(f"  → Start skipping: {line_upper}")
            return (True, line_upper)
        return (False, None)
    
    # We ARE in a skip section - check if we should end it
    if line_trimmed.startswith('## '):
        # Any new section ends the skip
        print(f"  → End skip at: {line_trimmed[:60]}")
        return (False, None)  # Process this line, end skipping
...
def convert_md_to_docx(md_file, output_file, template_path):
    ...
    while i < len(lines):
        ...
        should_skip, new_section_state = should_skip_section(line, current_skip_section)
        
        # Update the skip state
        current_skip_section = new_section_state
        
        # If we should skip this line, move to next
        if should_skip:
            i += 1
            continue
...
def main():
    ...
    documents = [
        ...
        os.path.join(parent_dir, "03_Regulatory_Process_Overview", "REG-PROC-001_Regulatory_Process_API_Export_Japan_Fampridin_v1.0.md"),
